                            ...
                            <div class="grid_unit_'.$unit['id'].'_update_form">
                                <form action="scripts/update_unit-script.php" method="post">
                                    <div class="row_1_inputs">
                                        ...
                                        <input type="text" id="unit_name_'.$unit['id'].'" value="'.$unit['unit_name'].'" placeholder="unit_name" name="unit_name">
                                        <label for="log_table">log_table</label>
                                        <input type="text" id="log_table_'.$unit['id'].'" value="'.$unit['log_table'].'" placeholder="log_table" name="log_table">
                                        <label for="delay_timer">delay_timer</label>
                                        <input type="number" id="delay_timer_'.$unit['id'].'" value="'.$unit['delay_timer'].'" placeholder="delay_timer" name="delay_timer">
                                        <label for="act_timer">act_timer</label>
                                        <input type="number" id="act_timer_'.$unit['id'].'" value="'.$unit['act_timer'].'" placeholder="act_timer" name="act_timer">
                                    </div>
                                    <div class="row_2_inputs">
                                        <label for="output_pin">output_pin</label>
                                        <input type="number" id="output_pin_'.$unit['id'].'" value="'.$unit['output_pin'].'" placeholder="output_pin" name="output_pin">
                                        <label for="led_pin">led_pin</label>
                                        <input type="number" id="led_pin_'.$unit['id'].'" value="'.$unit['led_pin'].'" placeholder="led_pin" name="led_pin">
                                        <label for="servo_extended">servo_extended</label>
                                        <input type="number" id="servo_extended_'.$unit['id'].'" value="'.$unit['servo_extended'].'" placeholder="servo_extended" name="servo_extended">
                                        <label for="servo_retracted">servo_retracted</label>
                                        <input type="number" id="servo_retracted_'.$unit['id'].'" value="'.$unit['servo_retracted'].'" placeholder="servo_retracted" name="servo_retracted">
                                        <label for="servo">servo</label>
                    ';

                    if ($unit['servo'] == '1')
                    {
                        echo '<input type="checkbox" id="servo_'.$unit['id'].'" name="servo" value="1" checked="checked">';
                    }
                    else
                    {
                        echo '<input type="checkbox" id="servo_'.$unit['id'].'" name="servo" value="1">';
                    }
                    
                    echo '<label for="toggle">toggle</label>';
                    if ($unit['toggle'] == '1')
                    {
                        echo '<input type="checkbox" id="toggle_'.$unit['id'].'" name="toggle" value="1" checked="checked">';
                    }
                    else
                    {
                        echo '<input type="checkbox" id="toggle_'.$unit['id'].'used" name="toggle" value="1">';
                    }
